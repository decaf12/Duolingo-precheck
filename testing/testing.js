...
class marking {
  ...
  const T = ({
    ...
    const [finalPath, finalWeight] = ((currState, endStateDetector, iterEdgesFunction, edgeWeightFunction, s) =>{
      ...
      // Dijkstra?
      for (; heap.size(); ) {
        const currTieBreaker = r.Fp(heap.toArray().map((({tieBreaker: e}) =>e))) + 1,
        {
          path: currPath,
          state: currState,
          weight: currWeight
        ...
        if (!visited[`${ currState }`]) {
          ...
          iterEdgesFunction(currState).forEach((([vertex, nextState], newTieBreaker) =>{
            const edgeWeight = edgeWeightFunction(vertex);
            if (edgeWeight < 0) {
              throw Error('Negative weight edge in solution DAG');
            }

            heap.push({
              path: [...currPath, [currState, vertex]],
              state: nextState,
              tieBreaker: currTieBreaker + newTieBreaker,
              weight: currWeight + edgeWeight
            })
          }))
        }
      };
